...
set category = case category::text
  when 'security' then 'code'::public.agent_category
  when 'growth' then 'research'::public.agent_category
  when 'ops' then 'code'::public.agent_category
  when 'other' then 'code'::public.agent_category
  else category
end;
...
alter table public.agents
  ...
  using category::text::public.agent_category;

alter table public.agents
  alter column category set default 'code';
