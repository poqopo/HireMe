alter table public.agents
  add column if not exists creator_info_url text,
  add column if not exists how_to_use text;

alter table public.agents
  drop constraint if exists agents_creator_info_url_http;

alter table public.agents
  add constraint agents_creator_info_url_http
  check (
    creator_info_url is null
    or creator_info_url ~* '^https?://[^[:space:]]+$'
  );

update public.agents
set how_to_use = 'Describe the Dokpami character variation you want. Include the outfit, prop, pose, expression, background, and whether the result should be a centered character asset or a full scene.'
where slug = 'dokpami-maker'
  and how_to_use is null;

update public.agents
set how_to_use = result_summary
