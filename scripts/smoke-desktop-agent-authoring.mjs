...
import { createDesktopAgentAuthoringService } from "../apps/desktop/agentAuthoring.mjs";
...
try {
  ...
  const designCreated = await service.createDraft({
    userId,
    input: {
      agentId: "brand-campaign-design-service",
      name: "Brand Campaign Design Service",
      category: "Design",
      headline: "Creates campaign visuals within a designer-defined decision system.",
      summary: "Uses customer answers while preserving the creator's priorities, boundaries, and quality bar.",
      skills: ["Art direction", "Brand quality checks"],
      resultTypes: ["Campaign image"],
      designSystem: {
        purpose: "Keep the brand trustworthy while making the campaign message readable at a glance.",
        priorities: ["Lead with the campaign message.", "Keep the product image visually dominant."],
        avoid: ["Decorative gradients that dilute the brand.", "Competing calls to action."],
        qualityBar: ["The message is readable in three seconds.", "Every brand restriction is honored."],
        questions: [
          {
            id: "campaign-goal",
            label: "What should the audience do first?",
            kind: "single",
            required: true,
            options: ["Learn more", "Buy now"],
          },
        ],
      },
    },
  });
  assert.equal(designCreated.status, "created");
  assert.ok(designCreated.revision > 1);
  const designSkillPath = join(
    userDataDir,
    "runtime",
    userId,
    "agents",
    designCreated.agentId,
    "skills",
    "design-decision-system",
    "SKILL.md",
  );
  const designSkill = await readFile(designSkillPath, "utf8");
  assert.ok(designSkill.includes("# Design Decision System"));
  assert.ok(designSkill.includes("## 결과 통과 기준"));
  assert.ok(designSkill.includes("What should the audience do first?"));
  const designPublicProfilePath = join(
    userDataDir,
    "runtime",
    userId,
    "agents",
  ...
  const designPublicProfile = JSON.parse(await readFile(designPublicProfilePath, "utf8"));
  assert.equal(designPublicProfile.design_contract.questions.length, 1);
  assert.equal(designPublicProfile.design_contract.priority_count, 2);

  const designManagement = await service.prepareManagement({
    userId,
    clientId,
    input: {
      conversationId: "conversation-design-system-update",
      agentId: designCreated.agentId,
      name: "Brand Campaign Design Service",
      category: "Design",
      headline: "Creates campaign visuals within a designer-defined decision system.",
      summary: "Uses customer answers while preserving the creator's priorities, boundaries, and quality bar.",
  ...
  const designUpdated = await service.updateDesignSystem({
    userId,
    clientId,
    input: {
      conversationId: designManagement.conversationId,
      agentId: designCreated.agentId,
      managementSessionId: designManagement.managementSession.id,
      designSystem: {
        purpose: "Keep the brand trustworthy and make the next action unmistakable.",
        priorities: ["Lead with the customer benefit."],
        avoid: ["Generic AI decoration."],
        qualityBar: ["One clear message is visible at a glance."],
        questions: [
          {
            id: "primary-message",
            label: "What is the one message the audience should remember?",
            kind: "short",
            required: true,
          },
        ],
  ...
  assert.ok(designUpdated.revision > designCreated.revision);
  assert.ok((await readFile(designSkillPath, "utf8")).includes("What is the one message the audience should remember?"));
  const updatedDesignPublicProfile = JSON.parse(await readFile(designPublicProfilePath, "utf8"));
  assert.equal(updatedDesignPublicProfile.design_contract.questions[0].id, "primary-message");
  service.revokeManagement({
    userId,
    clientId,
    input: {
      conversationId: designManagement.conversationId,
