...
import { createStandaloneAgent, loadStandaloneAgentProfile } from "../apps/agent/src/runtime.mjs";
import { validateLocalSpecialistAgent } from "../apps/agent/src/localSpecialistAgent.mjs";
...
try {
  ...
  const profile = await loadStandaloneAgentProfile("apps/agent/agents/hireme-operator");
  if (
...
function runAgentJson(args) {
  return new Promise((resolveRun, rejectRun) => {
    const child = spawn("node", ["apps/agent/src/index.mjs", ...args], {
      env: process.env,
      stdio: ["ignore", "pipe", "pipe"],
    });
    let stdout = "";
