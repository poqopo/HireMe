...
const providerResponseLimitError = createProcessExitError(
  "OpenAI Codex image generation response exceeded event limit.",
  0,
);
assert.equal(readRunErrorCode(providerResponseLimitError), "provider_response_limit");
assert.equal(providerResponseLimitError.exitCode, 0);
assert.equal(isTrustedRendererContext({
  senderId: 8,
  trustedWebContentsId: 7,
  senderUrl: devUrl,
