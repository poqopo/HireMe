...
```text
apps/design-harness/                 # 다음 구현 단계의 런타임 패키지
  contracts/                         # 이 폴더의 schema를 import
  project-store/                     # Project, Artifact, Trace 저장
  orchestrator/                      # workflow 상태 전이
  tool-gateway/                      # policy enforcement + audit
  evaluators/                        # machine, vision/LLM, human gate
  skills/                            # 검증된 manifest만 등록
```

현재 구현은 `apps/desktop/creatorWorker.mjs`, `apps/desktop/creatorWorkerExecutor.mjs`,
`supabase/functions/creator-worker`, `202608060001_creator_worker_control_plane.sql`에 연결돼
