...
`apps/agent`는 HireMe 데스크톱 앱과 CLI가 함께 사용하는 standalone Agent
runtime입니다.
...
native provider는 HireMe가 관리하는 ChatGPT OAuth profile을 사용해
`openai/gpt-image-2` image tool을 호출하고, 검증된 이미지 파일을 작업 폴더에
저장합니다. Codex CLI 실행 파일은 필요하지 않습니다. specialist는 먼저 safe
