import { completeOpenAICodexResponse } from "./openaiCodexProvider.mjs";
...
export function createModelProvider({
  ...
  if (["codex", "openai-codex"].includes(normalizedProvider)) {
    return createNativeOpenAICodexProvider({
      model: model || process.env.HIREME_AGENT_MODEL || process.env.HIREME_OPENAI_CODEX_RESPONSES_MODEL,
    });
  }
...
function createNativeOpenAICodexProvider({ model }) {
  const complete = async ({ instructions, input, signal }) => completeOpenAICodexResponse({
    instructions: [
      instructions,
      "",
  ...
  return {
    ...
    model: model || "openai-codex-default",
    complete,
