...
async function materializeCodexImageGenArtifact({
  ...
  try {
    ...
    return {
      type: "hireme_image_artifact_materialized",
      status: isImageRateLimitedError(err) ? "rate_limited" : "failed",
      provider: "codex_image_gen",
      validation,
      path: relative(workspaceRoot, filePath),
      bytes: 0,
      error: err?.message || String(err),
      retryAfterMs: Number(err?.retryAfterMs || 0) || null,
    };
  }
...
async function runCodexImageGenCommand(
  ...
  // The native bridge owns the network deadline. Keep the process watchdog
  // longer so it can return an actionable OAuth or API error instead of being
  // killed at the exact same millisecond as the inner request.
  const processTimeoutMs = timeoutMs + 60_000;
  return new Promise((resolveRun, rejectRun) => {
    ...
    child.on("exit", (exitCode) => {
      settle(() => {
        ...
        const result = parseCommandJson(stdout, stderr);
        if (result.status === "failed") {
          rejectRun(
            Object.assign(
              new Error(result.error || "Codex image_gen bridge command failed."),
              {
                code: result.code || "codex_image_gen_failed",
...
function isImageRateLimitedError(error) {
  return ["openai_codex_image_rate_limited", "openai_codex_image_slow_down"].includes(
    String(error?.code || ""),
  );
}
