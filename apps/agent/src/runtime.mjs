...
async function runStandaloneAgent({
  ...
  for (let iteration = 1; iteration <= limits.maxIterations; iteration += 1) {
    ...
    try {
      ...
      if (runtimeMode === "agent_authoring" && completesAuthoringTurn(tool.name)) {
        return completeAuthoringTurn({
          profile,
          model,
          memory,
          runId,
...
function completesAuthoringTurn(toolName) {
  return new Set([
...
async function completeAuthoringTurn({
  profile,
  model,
  memory,
  runId,
  ...
  const outputText = authoringToolSummary(toolName, observation, { exhausted });
  assertSafeOutput(profile, outputText);
  await memory?.writeEpisode?.({
    runId,
    goal,
  ...
  emitEvent({ type: "run_completed", iteration });
  return {
    type: "hireme_standalone_agent_result",
    status: "completed",
    runId,
    agent: { id: profile.id, name: profile.name, version: profile.version },
    provider: model.provider,
    model: model.model,
    outputText,
    iterationsRun: iteration,
...
function authoringToolSummary(toolName, observation, { exhausted = false } = {}) {
  if (exhausted) {
    return "요청 내용을 확인했어요. 이번 대화에서는 변경을 확정하지 않았습니다. 바꿀 대상이나 원하는 결과를 한 가지로 좁혀서 다시 알려주세요.";
  }
  const result = observation && typeof observation === "object" ? observation : {};
  if (toolName === "hireme_test_agent_draft") {
    const output = String(result?.result?.outputText || "").trim();
    const artifactCount = Array.isArray(result?.result?.artifacts) ? result.result.artifacts.length : 0;
    if (output) {
      return [
        "현재 초안으로 시험 실행을 완료했어요.",
        "",
        output,
        artifactCount ? `\n생성 파일 ${artifactCount}개도 결과에 포함했어요.` : "",
      ].filter(Boolean).join("\n");
    }
    return "현재 초안을 시험 실행했어요. 결과는 채팅의 생성 파일과 작업 기록에서 확인할 수 있어요.";
  }
  if (toolName === "hireme_update_agent_draft_file") {
    return "작업 방식을 업데이트하고 기본 검증을 통과했어요. 오른쪽 Private Harness에서 변경 내용을 확인한 뒤, `시험:`으로 시작해 결과를 바로 확인해 보세요.";
  }
  if (toolName === "hireme_create_agent_skill") {
    return "반복해서 사용할 작업 절차를 Agent의 비공개 스킬로 추가하고 검증했어요. 이제 같은 기준으로 시험 실행해 결과를 확인할 수 있어요.";
  }
  if (toolName === "hireme_add_agent_bootstrap_memory") {
    return "배포 시 함께 전달될 기본 기억을 업데이트하고 검증했어요. 실제 사용자 대화에서는 세션 기억이 이 기본 기억보다 우선 적용됩니다.";
  }
  if (toolName === "hireme_evaluate_agent_draft") {
    return "현재 초안의 평가를 마쳤어요. 결과에 따라 작업 방식이나 기본 기억을 한 번 더 조정할 수 있어요.";
  }
  if (toolName === "hireme_validate_agent_draft") {
    return "현재 초안의 구조와 기본 규칙을 확인했어요. 이제 `시험:`으로 시작해 실제 결과를 확인할 수 있어요.";
  }
  return "관리 작업을 처리했어요. Private Harness 원문은 채팅에 표시하지 않았습니다.";
}
