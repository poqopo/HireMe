...
async function runCommandSpecialist(agent, input, { signal } = {}) {
  ...
  // Packaged Electron apps do not ship a separate `node` binary. Their own
  // executable becomes the Node runtime when ELECTRON_RUN_AS_NODE is set.
  const executable = command === "node" && process.env.ELECTRON_RUN_AS_NODE === "1"
    ? process.execPath
    : command;
