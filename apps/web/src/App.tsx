...
type ViewId = "chat" | "discover" | "agents" | "earnings" | "review";
...
function HireMeWorkspace({
  ...
  const workScopeCounts: Record<WorkScope, number> = {
    created: recentConversations.filter((conversation) => (
      workScopeForConversation(conversation, agents) === "created"
    )).length,
    hired: recentConversations.filter((conversation) => (
      workScopeForConversation(conversation, agents) === "hired"
    )).length,
  };
  const runForWorkScope = (scope: WorkScope) => {
    const runningConversation = recentConversations.find((conversation) => (
      workScopeForConversation(conversation, agents) === scope && Boolean(runs[conversation.id])
    ));
    return runningConversation ? runs[runningConversation.id] : undefined;
  };
  const workScopeRuns: Record<WorkScope, RunState | undefined> = {
    created: runForWorkScope("created"),
    hired: runForWorkScope("hired"),
  };
  ...
  const switchWorkScope = (nextScope: WorkScope) => {
    const nextConversation = recentConversations.find((conversation) => (
      workScopeForConversation(conversation, agents) === nextScope
    ));
    if (
      (view !== "chat" || nextConversation?.id !== activeConversation?.id) &&
      !confirmDiscardManagementDraft(activeConversation?.id)
    ) return false;
    ...
    <div className={`desktop-app ${managementInspectorOpen ? "agent-authoring-mode" : ""} ${inspectorClosed ? "inspector-closed" : ""} ${view === "earnings" ? "earnings-coming-soon-mode" : ""}`} data-native={nativeRuntime ? "true" : "false"}>
      ...
      <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
        ...
        <nav className="primary-nav" aria-label="주요 메뉴">
          <span className="sidebar-nav-label">관리</span>
          <button
            type="button"
          ...
          >
            <Target size={17} />
            <span><strong>내 디자인 서비스</strong><small>판단 기준 · 질문 · 수익</small></span>
            <small className="sidebar-count">{ownedAgents.length}</small>
          </button>
          <button
            type="button"
            className={view === "discover" ? "nav-item nav-item-rich active" : "nav-item nav-item-rich"}
            onClick={() => {
              if (navigateToView("discover")) setSidebarOpen(false);
            }}
          >
            <Compass size={17} />
            <span><strong>디자인 서비스 찾기</strong><small>전문가가 설계한 서비스</small></span>
          </button>
          <span className="sidebar-nav-label conversation-nav-label">작업</span>
          <button
            type="button"
            className={view === "chat" && workScope === "created" ? "nav-item nav-item-rich active" : "nav-item nav-item-rich"}
            onClick={() => {
              if (switchWorkScope("created")) setSidebarOpen(false);
            }}
          >
            <Sparkles size={17} />
            <span><strong>내 서비스 테스트</strong><small>고객 질문과 결과 확인</small></span>
            {workScopeRuns.created
              ? <WorkScopeRunStatus key={workScopeRuns.created.runId} run={workScopeRuns.created} />
              : <small className="sidebar-count">{workScopeCounts.created}</small>}
          </button>
          <button
            type="button"
            className={view === "chat" && workScope === "hired" ? "nav-item nav-item-rich active" : "nav-item nav-item-rich"}
            onClick={() => {
              if (switchWorkScope("hired")) setSidebarOpen(false);
            }}
          >
            <BriefcaseBusiness size={17} />
            <span><strong>맡긴 디자인 작업</strong><small>질문에 답하고 결과 받기</small></span>
            {workScopeRuns.hired
              ? <WorkScopeRunStatus key={workScopeRuns.hired.runId} run={workScopeRuns.hired} />
              : <small className="sidebar-count">{workScopeCounts.hired}</small>}
          </button>
        </nav>
...
function WorkScopeRunStatus({ run }: { run: RunState }) {
  const elapsed = useElapsed(run.startedAt);
  const label = formatElapsed(elapsed);
  return (
    <span className="sidebar-run-status" role="timer" aria-label={`작업 경과 시간 ${label}`}>
      <LoaderCircle className="spin run-activity-spinner" size={12} />
      <small>{label}</small>
    </span>
  );
}
      ...
      <aside className="design-work-thread">
        ...
        <div className="workspace-composer">
          ...
          <div>
            ...
            <button className="send-button" type="button" aria-label={run ? "수정 요청 대기열에 추가" : "수정 요청 보내기"} disabled={!draft.trim() && attachments.length === 0} onClick={onSubmit}>
              <Send size={16} />
            </button>
          </div>
        </div>
...
function DesignCanvasResult({
  ...
  if (artifact && isImageFile(artifact) && source) {
    return (
      <button
        className="design-canvas-image"
        type="button"
        disabled={!artifact.path}
        title={artifact.path ? `${artifact.name} 원본 열기` : artifact.name}
        onClick={() => openWorkspaceFile(artifact)}
      >
        <img src={source} alt={artifact.name} onError={onError} />
      </button>
    );
  }
...
function DiscoverView({
  ...
  const [category, setCategory] = useState<"전체" | AgentCategory>("디자인");
  const [sort, setSort] = useState<"추천순" | "인기순" | "평점순">("추천순");
  const filtered = agents
    .filter((agent) => category === "전체" || agent.category === category)
    .filter((agent) => [agent.name, agent.creator, agent.headline, ...agent.skills].join(" ").toLowerCase().includes(query.toLowerCase()))
    .sort((a, b) => (sort === "인기순" ? b.uses - a.uses : sort === "평점순" ? b.rating - a.rating : Number(b.hired) - Number(a.hired)));

  return (
    <section className="page-view discover-view">
      <header className="page-header">
        <div>
          <span className="eyebrow">Design service directory</span>
          <h1>디자이너의 기준으로 완성되는 서비스를 선택하세요</h1>
          <p>빈 프롬프트 대신 전문가가 설계한 질문에 답하고, 판단 기준을 통과한 결과를 받습니다.</p>
        </div>
      </header>
      ...
      <div className="discover-toolbar">
        <label className="search-field">
          <Search size={17} />
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="필요한 결과나 디자인 서비스 검색" />
        </label>
        <div className="category-tabs" role="tablist" aria-label="카테고리">
          {(["전체", "디자인", "글쓰기", "비즈니스", "리서치", "생산성"] as const).map((item) => (
            <button type="button" role="tab" aria-selected={category === item} className={category === item ? "active" : ""} key={item} onClick={() => setCategory(item)}>
              {item}
            </button>
          ))}
        </div>
      ...
      <div className="result-summary">
        <span>{filtered.length}개의 디자인 서비스</span>
        <span><ShieldCheck size={14} /> 디자이너의 세부 판단 기준은 비공개로 보호됩니다</span>
      </div>
        ...
        {filtered.map((agent) => (
          <article
            className={[
              ...
              agent.ownership === "mine" ? "owned-agent-card" : "",
            ].filter(Boolean).join(" ")}
            key={agent.id}
            ...
            <div className="agent-card-body">
              <div className="agent-card-title">
                ...
                {agent.ownership === "mine"
                  ? <span className="owned-agent-badge"><Check size={12} /> 내 서비스</span>
                  : agent.hired && <span className="hired-badge"><Check size={12} /> 이용 중</span>}
              </div>
              <p>{agent.headline}</p>
              <div className="skill-row">
                {agent.skills.slice(0, 3).map((skill) => <span key={skill}>{skill}</span>)}
              </div>
              {agent.designSystem && <div className="design-service-proof"><MessageCircleQuestion size={13} /> 질문 {agent.designSystem.questions.length}개 <span /> <ListChecks size={13} /> 자동 품질 검사</div>}
              <div className="agent-card-stats">
                <span><Star size={14} fill="currentColor" /> {agent.rating} <small>({agent.reviews})</small></span>
                <span><Users size={14} /> {formatCompact(agent.uses)}건</span>
              </div>
            </div>
            <div className="agent-card-footer">
              <span><strong>가격 : <PendingPrice agent={agent} /></strong><small>{agent.creator}</small></span>
              <button className={agent.ownership === "mine" || agent.hired ? "primary-button compact" : "secondary-button compact"} type="button" onClick={(event) => { event.stopPropagation(); onUse(agent.id); }}>
                {agent.ownership === "mine" ? "테스트하기" : agent.hired ? "질문에 답하기" : "서비스 이용하기"}
              </button>
            </div>
          </article>
        ))}
...
function MyAgentsView({
  ...
  onWork: (agentId: string) => void;
}) {
  const totalRevenue = agents.reduce((sum, agent) => sum + (agent.revenue30d || 0), 0);
  const totalUses = agents.reduce((sum, agent) => sum + agent.uses, 0);
  return (
    <section className="page-view my-agents-view">
      ...
      <header className="page-header split">
        <div>
          ...
          <h1>내 디자인 서비스</h1>
          <p>디자인 판단 기준과 고객 질문을 설계하고, 반복 가능한 서비스로 운영하세요.</p>
        </div>
        <div className="header-actions">
          <button className="secondary-button" type="button" onClick={onOpenEarnings}><Wallet size={16} /> 수익 관리</button>
          <button className="primary-button" type="button" onClick={onCreate}><Plus size={16} /> 새 디자인 서비스</button>
        </div>
      </header>
      ...
      <div className="metric-strip three">
        ...
        <Metric label="자동 납품" value={`${formatCompact(totalUses)}건`} detail="기준 통과율 97.8%" icon={<ListChecks size={17} />} />
      </div>
          ...
          <tbody>
            {agents.map((agent) => (
              <tr key={agent.id} className={selectedAgentId === agent.id ? "selected" : ""} onClick={() => onSelect(agent.id)}>
                ...
                <td>{formatCompact(agent.uses)}건</td>
                <td>
                  <div className="table-row-actions">
                    ...
                    <button className="icon-button" type="button" aria-label={`${agent.name} 작업 모드`} title="작업 모드" onClick={(event) => { event.stopPropagation(); onWork(agent.id); }}><Play size={15} /></button>
...
function NewAgentDialog({ onClose, onCreate }: { onClose: () => void; onCreate: (agent: Agent) => Promise<void> }) {
  ...
  return (
    <Dialog title="새 디자인 서비스 만들기" subtitle="고객이 답할 질문과 디자이너의 판단 기준을 묶어 반복 가능한 서비스로 만듭니다." onClose={onClose} wide scrollable>
      <form className="agent-form edit-agent-form" onSubmit={submit}>
        ...
        <label><span>이름</span><input autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="예: 브랜드 제안서 도우미" required disabled={busy} /></label>
        <label><span>한 줄 소개</span><input value={headline} onChange={(event) => setHeadline(event.target.value)} placeholder="예: 의뢰 내용을 설득력 있는 제안서로 정리해요" required disabled={busy} /></label>
        <label><span>상세 설명</span><textarea value={summary} onChange={(event) => setSummary(event.target.value)} placeholder="누구의 어떤 문제를 해결하고, 어떤 결과를 잘 만드는지 알려주세요." required disabled={busy} /></label>
        <fieldset disabled={busy}><legend>분야</legend><div className="segmented-options">{(["디자인", "글쓰기", "비즈니스", "리서치", "생산성"] as AgentCategory[]).map((item) => <button className={category === item ? "active" : ""} type="button" key={item} onClick={() => setCategory(item)}>{item}</button>)}</div></fieldset>
        <label><span>잘하는 일</span><input value={skills} onChange={(event) => setSkills(event.target.value)} placeholder="제안서, 견적 문구, 범위 정리" disabled={busy} /><small>쉼표로 구분</small></label>
        {category === "디자인" && <DesignSystemEditor value={designSystem} onChange={setDesignSystem} disabled={busy} />}
        <label><span>실행당 가격</span><input type="text" inputMode="numeric" value={runPrice} onChange={(event) => setRunPrice(formatRunPriceInput(event.target.value))} placeholder="예: 1,900" required disabled={busy} /><small>{runPrice ? `${formatWon(normalizeRunPrice(runPrice))} / 실행` : "사용자가 에이전트를 한 번 실행할 때 받을 금액(원)입니다."} · 수익 기능 준비 중</small></label>
        ...
        <div className="dialog-actions">
          <button className="secondary-button" type="button" onClick={onClose} disabled={busy}>취소</button>
          <button className="primary-button" type="submit" disabled={!valid || busy}>{busy ? <><LoaderCircle className="spin" size={15} /> 초안 만드는 중</> : "초안 만들기"}</button>
        </div>
      </form>
    </Dialog>
  );
