...
비개발자 프리랜서가 별도 설정 없이 반복 업무를 맡길 수 있는 기본 제품
화면입니다.

- 여러 채팅을 만들고 작업별로 전환
- 고용한 에이전트와 내가 만든 에이전트를 한곳에서 사용
- 작업 폴더와 파일 첨부
...
ChatGPT 계정(Codex) 또는 이 컴퓨터의 Ollama를 선택합니다. 이 연결은 작업을
...
받고 UUID별 상태 폴더를 사용합니다. Codex OAuth 파일도 UUID별 로컬 폴더에
격리되며 DB에는 선택한 AI와 최초 설정 완료 여부만 저장됩니다. Codex 연결은
텍스트 실행과 `gpt-image-2` 이미지 생성에 함께 사용됩니다.
