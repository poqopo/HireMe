...
export function approveSealAccess({
  ...
  return {
    ...
    provider: sealMetadata?.provider || "platform-managed-envelope",
    ...
    note:
      "MVP uses platform-managed decryption after paid receipt verification. Seal key servers are an optional later provider.",
  };
}
...
export function decryptSealEnvelope({
  ...
  if (!approval?.sealEncryptionId || approval.sealEncryptionId !== encryptionId) {
    throw userError("Seal approval does not match the encrypted object identity");
  }
  ...
  if (dem.algorithm !== "aes-256-gcm") {
    throw userError(`Unsupported Seal DEM algorithm: ${dem.algorithm || "unknown"}`);
  }
