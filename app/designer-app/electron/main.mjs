...
const agentRuntimeRoot = app.isPackaged
  ? join(process.resourcesPath, "runtime", "hireme-agent")
  : join(developmentRoot, "hireme-agent");
const {
  isManagementEscalationRequest,
  managementModeRequiredMessage,
} = await import(pathToFileURL(join(agentRuntimeRoot, "runtime", "src", "managementModePolicy.mjs")).href);
...
function rendererPath() {
  return join(app.getAppPath(), "renderer/dist/index.html");
}
...
async function readLocalAgentCards() {
  const agentRoot = join(runtimeRoot(), "hireme-agent", "examples", "local-specialist-agents");
