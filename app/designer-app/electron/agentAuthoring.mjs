...
export function createDesktopAgentAuthoringService({
  ...
  const loadAuthoringModule = () => {
    authoringModulePromise ||= import(pathToFileURL(
      join(resolve(runtimeRoot), "hireme-agent/runtime/src/agentAuthoringLayer.mjs"),
    ).href);
  ...
  return {
    ...
    async prepareManagement({ userId, clientId, input } = {}) {
      ...
      if (!exists) {
        ...
        const bundledAgentRoot = join(resolve(runtimeRoot), "hireme-agent/examples/local-specialist-agents", draft.agentId);
