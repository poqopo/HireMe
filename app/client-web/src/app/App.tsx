...
export function App() {
  ...
  const nextFromRequest = () => { setSelectedAgent(rankedAgents[0]); setStep("match"); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  const restart = () => { setRequest(initialRequest); setSelectedAgent(undefined); setSubmitted(undefined); setError(""); navigate("request"); };

  return <div className="min-h-screen bg-[#faf9f6] text-stone-950">
    <SiteHeader signedIn={Boolean(session)} onLogin={() => void login()} onProjects={() => setShowProjects(true)} />
