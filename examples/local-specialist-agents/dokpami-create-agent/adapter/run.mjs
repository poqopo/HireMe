...
prompt = prompt_builder.build_prompt(payload["request"], payload["mode"], payload["characterOnly"])
print(json.dumps({
    "sha256": hashlib.sha256(prompt.encode("utf-8")).hexdigest(),
    "bytes": len(prompt.encode("utf-8")),
}, ensure_ascii=False))
`;
  const payload = JSON.stringify({ request, mode, characterOnly });
  let lastError = null;
  for (const command of pythonCommandCandidates()) {
    try {
      const result = await runCommand(command, ["-c", code], {
        cwd: privateSourceRoot,
        input: payload,
        timeoutMs: 20_000,
      });
  ...
  throw lastError || new Error("A Python 3 runtime is required by the private prompt builder.");
}

function pythonCommandCandidates() {
  return [...new Set([
    process.env.HIREME_PYTHON_COMMAND,
    process.platform === "darwin" ? "/usr/bin/python3" : null,
    process.platform === "darwin" ? "/opt/homebrew/bin/python3" : null,
    process.platform === "darwin" ? "/usr/local/bin/python3" : null,
    "python3",
    "python",
  ].filter(Boolean))];
}
